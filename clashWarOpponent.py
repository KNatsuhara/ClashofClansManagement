...
# Clan Tag
clan_tag = "2QU0V8PP2" # opponent clan tag that will retrieve data from their log about OUR MEMBERS
...
def populateWarList(war_json):
    count = 0
    for member in war_json["opponent"]["members"]:
        total_stars = 0
        total_percent = 0
        count += 1
        print(member["name"])
...
def createFilename(war_json):
    ...
    clan_name = war_json["opponent"]["name"]
    opponent_clan_name = war_json["clan"]["name"]
